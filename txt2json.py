...
def txt2json(root_path,category_dict, json_path):
    ...
    for k, index in enumerate(tqdm.tqdm(indexes)):
        ...
        with open(os.path.join(originLabelsDir, txtFile), 'r') as fr:
            ...
            for label in labelList:
                ...
                if int(label[0]) == 0 or int(label[0]) == 11:
                    continue
                x = float(label[1])
                y = float(label[2])
                w = float(label[3])
    ...
    #     os.makedirs(folder)
    json_name = os.path.join(root_path, '{}'.format(json_path))
...
if __name__ == "__main__":
    ...
    #python txt2json.py -rp VisDrone2019-DET-val -jp VisDrone2019-DET-val-coco.json
    parse = argparse.ArgumentParser("VisDrone")
    parse.add_argument("-jp",type=str, default=None)
    parse.add_argument("-rp",type=str, default=None)
    args = parse.parse_args()

    root_path = "/home/zhsong/Code/datasets/VisDrone2019-DET/"
    root_path = os.path.join(root_path,args.rp)
    print(root_path,args.jp)
    json_path = args.jp
    category_dict = {
        'pedestrian': 1, 
        'people': 2,
